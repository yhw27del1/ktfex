	 ...
	 <input type='hidden' class='autoheight' value="auto" />  
		...
		<div style="width:900px;overflow-x:hidden;margin-left:auto;margin-right:auto;margin-top:30xp;"  id="maindiv">
			<!-- 
				...
				<div><h1 class="bold center">借款合同-----企业投资模板X<input id="financingBaseId" type="hidden" value="${financingBaseId}"/></h1></div>
				...
				<div class="l4">2. 按月等额本息还款： 等额每月还本金： <span class="key"><c:if test="${contract.payment_method==2}">_</c:if></span>元（大写： <span class="key"><c:if test="${contract.payment_method==2}">${contract.principal_uppercase_monthly}</c:if></span>），等额每月还利息： <span class="key"><c:if test="${contract.payment_method==2}">_</c:if></span>元（大写： <span class="key"><c:if test="${contract.payment_method==2}">_</c:if></span>）。</div>
				<div class="l4">3. 按月等额付息，到期一次还本： 等额每月还息： 元（大写： ）。</div>
				<div class="l2"><span class="bold">第四条</span> 若上条约定的还款方式为按月等额本息还款，则每月 <span class="key">${contract.repayment_term==0?"":contract.repayment_term}</span>日为还款日，每月应还金额合计为： <span class="key">_</span>元（大写： <span class="key">_</span>）。若还款日逢国家法定节假日则当月还款日提前至该假期前的最后一个工作日。若当月无该日期的，当月还款日提前至该日期前的最后一个工作日。</div>
				...
				<div class="l2">一、乙方出现本合同第二十条约定的情形或违反本合同其他约定的，最终导致丙方解除本合同、提前收回全部借款本息时，除结清全部借款本金及利息（计算至结清日当天）外，乙方应另行向甲方支付借款金额的5%作为违约金，若违约金不足以偿付甲方、丙方因索赔而产生的各项费用时，甲方和丙方都有权继续向乙方索赔。</div>
				<div class="l2">二、乙方未按本合同约定按时、足额偿还借款本息的，自逾期之日起应向甲方每日按应还款金额的0.1%支付滞纳金。</div>
				<div class="l2">三、在本合同项下的借款本息全部清偿前，乙方及乙方的联系方式、实际居住地、工作单位、资产、负债等情况发生变化，未在变更后三天内书面通知丙方的，上述行为每发生一次，乙方应向丙方支付500元/项（次）的违约金。</div>
				<div class="l2">四、在本合同履行期间，乙方拒不履行还款义务或无力清偿欠款，而最终导致甲方付诸法律的，除承担全部费用外，乙方还应向甲方支付借款额的5%作为违约金。</div>
				...
				<div class="l2"><span class="bold">第二十三条</span> 本合同履行期间，如果乙方公司转让、变更等必须征得丙方的书面同意，否则转让行为无效。丙方有权直接通知甲方及其他全部出借人解除与乙方的借款合同、提前收回全部借款本息，同时乙方向甲方及丙方各支付人民币10000元的违约金。</div>
