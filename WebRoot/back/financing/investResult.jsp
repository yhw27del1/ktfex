...
<div class="ui-widget"  style="font-size:11pt;font-family: '宋体'">
	<div style="margin-top: 5px; padding: 0 .7em;" class="ui-state-highlight ui-corner-all"> 
		<p>
			您的交易账户余额为：<span id="balance">${account.balance}</span>元<br />
			<c:if test='${invest.investFlag==false}'>
				<span style='color:red;'>${invest.msg}<button class="ui-state-default" id="submitBtn">返回</button></span>
			</c:if>
			<c:if test='${invest.investFlag==true}'>
				<span style='color:red;'>本次投标成功。<button class="ui-state-default" id="submitBtn">返回</button></span>
				投标金额:${record.investAmount}元，投标前账户余额:${record.preAmount}元，投标后账户余额：${record.nextAmount}，投资服务费：${cost.tzfwf}元
			</c:if>
		</p>
	</div>
</div> 
			  ...
			  <table  style="width:70%;font-size:10p;" class="s_table">  
                ...
                <tr>
				 ...
				 <tr>
					...
					<td > ${financingBase.fxbzStateName}
					</td>
			...
			<dl>
				<dt>
					<a>风险评级</a><a>保障措施</a><a>项目详情</a><c:if test="${financingBase.fxbzState=='1'}"><a>担保情况</a></c:if><a>资料清单</a>
				</dt>
