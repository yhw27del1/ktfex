	 ...
	 <input type='hidden' class='autoheight' value="auto" />  
		...
		<div style="width:900px;overflow-x:hidden;margin-left:auto;margin-right:auto;margin-top:30xp;">
			<!-- 
				...
				<div class="l2"><span class="bold">第一条</span> 甲方通过丙方向乙方提供借款人民币： <span class="key"><fmt:formatNumber value="${contract.loan_allah}" pattern="0.00"/></span>元（大写： <span class="key">${contract.loan_uppercase}</span>）。</div>
				...
				<div class="l2"><span class="bold">第二条</span> 借款年利率为： <span class="key">${contract.interest_rate}%</span>。利息总额为： <span class="key"><fmt:formatNumber value="${contract.interest_allah}" pattern="0.00"/></span>元（大写：<span class="key">${contract.interest_uppercase} </span>）。</div>
				...
				<div class="l4">2. 按月等额本息还款： 等额每月还本金： <span class="key"><c:if test="${contract.payment_method==2}"><fmt:formatNumber value="${contract.principal_allah_monthly}" pattern="0.00"/></c:if></span>元（大写： <span class="key"><c:if test="${contract.payment_method==2}">${contract.principal_uppercase_monthly}</c:if></span>），等额每月还利息： <span class="key"><c:if test="${contract.payment_method==2}"><fmt:formatNumber value="${contract.interest_allah_monthly}" pattern="0.00"/></c:if></span>元（大写： <span class="key"><c:if test="${contract.payment_method==2}">${contract.interest_uppercase_monthly}</c:if></span>）。</div>
				<div class="l4">3. 按月等额付息，到期一次还本： 等额每月还息：  <span class="key"><c:if test="${contract.payment_method==3}"><fmt:formatNumber value="${contract.interest_allah_monthly}" pattern="0.00"/></c:if></span>元（大写：<span class="key"><c:if test="${contract.payment_method==3}">${contract.interest_uppercase_monthly}</c:if></span> ）。</div>
				<div class="l2"><span class="bold">第四条</span> 若上条约定的还款方式为按月等额本息还款，则每月 <c:if test="${contract.payment_method==2}"><span class="key">${contract.repayment_term!=0&&contract.payment_method==2?contract.repayment_term:""}</span></c:if>日为还款日，每月应还金额合计为： <c:if test="${contract.payment_method==2}"><span class="key"><fmt:formatNumber value="${contract.repayment_amount_monthly_allah}" pattern="0.00"/></span></c:if>元（大写： <c:if test="${contract.payment_method==2}"><span class="key">${contract.repayment_amount_monthly_uppercase}</span></c:if>）。若还款日逢国家法定节假日则当月还款日提前至该假期前的最后一个工作日。若当月无该日期的，当月还款日提前至该日期前的最后一个工作日。</div>
				...
				%>
				<div class="l2">一、风险管理费： <span class="key"><fmt:formatNumber value="${contract.riskmanagement_cost_all_allah}" pattern="0.00"/></span>元（大写： <span class="key"><%=fxglf_upl %></span>）。</div>
				<div class="l2">二、融资服务费： <span class="key"><fmt:formatNumber value="${contract.service_cost_all_allah}" pattern="0.00"/></span>元（大写： <span class="key"><%=rzfwf_upl %></span>）。</div>
				<div class="l2">合计： <span class="key"><fmt:formatNumber value="${contract.riskmanagement_cost_all_allah+contract.service_cost_all_allah}" pattern="0.00"/></span>元（大写： <span class="key"><%=all_upl %></span>）。</div>
