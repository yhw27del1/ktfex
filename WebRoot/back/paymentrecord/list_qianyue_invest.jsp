...
<body>
<form action="paymentRecord/paymentRecordAction!list_qianyue_invest">
...
<div class="dataList ui-widget">
	<table>
		<thead>
			<tr>
				<th style="text-align: center;font-size:22px;">昆投互联网金融交易-${bank}签约用户投标记录</th>
			</tr>
		</thead>
		<tbody>
			<table class="ui-widget ui-widget-content" id="mytable" style="font-size:12px;">
				...
				<c:forEach items="${pageView}" var="iter">
					<tr>
						<td><fmt:formatDate value="${iter.financingBase.qianyueDate}" type="both"/></td>
						<td><fmt:formatNumber value="${iter.investAmount}" pattern="0.00"/>元</td>
						<td>
							${iter.investor.user.username}/
							<c:if test="${iter.investor.category=='0'}">
								${iter.investor.eName}
							</c:if>
							<c:if test="${iter.investor.category!='0'}">
								${iter.investor.pName}
							</c:if>/${iter.investor.banklib.caption}/${iter.investor.user.accountNo}
						</td>
						<td>
							<fmt:formatDate value="${iter.investor.user.signDate}" type="both"/>
						</td>
						<td>${iter.financingBase.financier.user.username}/
						<c:if test="${iter.financingBase.financier.category=='1'}">
							${iter.financingBase.financier.pName}
						</c:if>
						<c:if test="${iter.financingBase.financier.category!='1'}">
							${iter.financingBase.financier.eName}
						</c:if>/${iter.financingBase.financier.bankAccount}
						</td>
						<td>${iter.financingBase.code}</td>
						<td>
					<c:if test="${(iter.financingBase.interestDay)!= 0}">按日计息(${iter.financingBase.interestDay}天)</c:if>
			        <c:if test="${(iter.financingBase.interestDay)== 0}">${iter.financingBase.businessType.term}个月 </c:if>    
