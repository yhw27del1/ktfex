		...
		<table width="100%">
			<thead>
				<tr style="background:#eeeeee">
					...
					<th rowspan="2">笔数</th>
					<th colspan="3">充值金额</th>
					<th rowspan="2">笔数</th>
					<th colspan="3">提现金额</th>
				</tr>
				<tr style="background:#eeeeee">
					<th>招行</th>
					<th>工行</th>
			...
			<tbody>
				...
				<c:set value="0" var="cz_je_hj_cmb"/>
				<c:set value="0" var="cz_je_hj_icbc"/>
				<c:set value="0" var="tx_bs_hj"/>
				<c:set value="0" var="tx_je_hj"/>
				<c:set value="0" var="tx_je_hj_cmb"/>
				<c:set value="0" var="tx_je_hj_icbc"/>
				
				
				<c:forEach items="${list}" var="item">
					<c:set value="0" var="cz_bs_xj"/>
					<c:set value="0" var="cz_je_xj"/>
					<c:set value="0" var="cz_je_xj_cmb"/>
					<c:set value="0" var="cz_je_xj_icbc"/>
					<c:set value="0" var="tx_bs_xj"/>
					<c:set value="0" var="tx_je_xj"/>
					<c:set value="0" var="tx_je_xj_cmb"/>
					<c:set value="0" var="tx_je_xj_icbc"/>
					<c:forEach items="${item.value}" var="item_i" varStatus="state">
						<tr>
							...
							<c:if test="${fn:contains(item_i.key,'提现')}">
								...
								<td>
									<fmt:formatNumber value="${item_i.value.icbc == null ? 0 : item_i.value.icbc}" pattern="#,##0.00"/>
								</td>
								<td>
									<fmt:formatNumber value="${item_i.value.sum == null ? 0 : item_i.value.sum}" pattern="#,##0.00"/>
								</td>
								<c:set value="${tx_bs_xj+item_i.value.count}" var="tx_bs_xj"/>
								<c:set value="${tx_je_xj+item_i.value.sum}" var="tx_je_xj"/>
								<c:set value="${tx_je_xj_cmb+item_i.value.cmb}" var="tx_je_xj_cmb"/>
								<c:set value="${tx_je_xj_icbc+item_i.value.icbc}" var="tx_je_xj_icbc"/>
							</c:if>
							...
							<c:if test="${fn:contains(item_i.key,'提现') == false}">
								...
								<c:set value="${cz_je_xj+item_i.value.sum}" var="cz_je_xj"/>
								<c:set value="${cz_je_xj_cmb+item_i.value.cmb}" var="cz_je_xj_cmb"/>
								<c:set value="${cz_je_xj_icbc+item_i.value.icbc}" var="cz_je_xj_icbc"/>
					...
					<tr style="background:#eeeeee">
						<td colspan="2">小计</td>
						<td>${cz_bs_xj}</td>
						<td><fmt:formatNumber value="${cz_je_xj_cmb}" pattern="#,##0.00"/></td>
						<td><fmt:formatNumber value="${cz_je_xj_icbc}" pattern="#,##0.00"/></td>
						<td><fmt:formatNumber value="${cz_je_xj}" pattern="#,##0.00"/></td>
						<td>${tx_bs_xj}</td>
						<td><fmt:formatNumber value="${tx_je_xj_cmb}" pattern="#,##0.00"/></td>
						<td><fmt:formatNumber value="${tx_je_xj_icbc}" pattern="#,##0.00"/></td>
						...
						<c:set value="${cz_je_xj_cmb + cz_je_hj_cmb}" var="cz_je_hj_cmb"/>
						<c:set value="${cz_je_xj_icbc + cz_je_hj_icbc}" var="cz_je_hj_icbc"/>
						<c:set value="${tx_bs_xj + tx_bs_hj}" var="tx_bs_hj"/>
						<c:set value="${tx_je_xj + tx_je_hj}" var="tx_je_hj"/>
						<c:set value="${tx_je_xj_cmb + tx_je_hj_cmb}" var="tx_je_hj_cmb"/>
						<c:set value="${tx_je_xj_icbc + tx_je_hj_icbc}" var="tx_je_hj_icbc"/>
					</tr>
				</c:forEach>
			</tbody>
			<tfoot>
				<tr style="background:#eeeeee">
					<th colspan="2">合计</th>
					<th>${cz_bs_hj}</th>
					<th><fmt:formatNumber value="${cz_je_hj_cmb }" pattern="#,##0.00"/></th>
					<th><fmt:formatNumber value="${cz_je_hj_icbc }" pattern="#,##0.00"/></th>
					<th><fmt:formatNumber value="${cz_je_hj }" pattern="#,##0.00"/></th>
					<th>${tx_bs_hj }</th>
					<th><fmt:formatNumber value="${tx_je_hj_cmb }" pattern="#,##0.00"/></th>
					<th><fmt:formatNumber value="${tx_je_hj_icbc }" pattern="#,##0.00"/></th>
