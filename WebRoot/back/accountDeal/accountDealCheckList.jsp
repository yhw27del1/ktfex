    	...
    	if(c!="batch"){
    		...
    		$.dialog({title:'提示信息',lock:true,min:false,max:false,width:'400px',content:"你确定要执行批量审核通过吗？",
				ok:function(){
					...
					window.location.href = "/back/accountDeal/accountDealAction!chargeCheckList_batch_check?time="+new Date().getTime()+"&keyWord="+encodeURI(k)+"&chargeStyle="+c+"&channel=1";
				},
    	...
    	if(c!="batch"){
    		...
    		$.dialog({title:'提示信息',lock:true,min:false,max:false,width:'400px',content:"你确定要执行批量审核驳回吗？",
				ok:function(){
					...
					window.location.href = "/back/accountDeal/accountDealAction!chargeCheckList_batch_check_no?time="+new Date().getTime()+"&keyWord="+encodeURI(k)+"&chargeStyle="+c+"&channel=1";
				},
				cancelVal:'关闭',cancel:true
			});
    	}
			...
			ok:function(){
				if(flag=="你确定要将此笔充值审核驳回吗？"){
					$.dialog.prompt("请输入充值驳回原因",function(val){
						window.location.href = url+"&memo="+val+"&channel=1";
					});
				}else{
...
<body> 
<input type='hidden' class='autoheight' value="auto" />   <!--待审核招行充值列表  -->
<form action="">
	<div id="myToolBar"
