...
$(function(){
	...
	$("#query").click(function(){
		...
		if(value == ""){
			...
			$.getJSON("/back/accountDeal/accountDealAction!query?time=" + new Date().getTime()+"&value="+value+"&type="+type+"&channel=1", function(data){
				$("#chager_list tr").remove();
				if(data.flag==false){
					alert("缴费失败，请重新查询。");
				}else{
					$.each(data,function(key,record){
						if('投资人'==record.tip){
							$("<tr><td>"+record.username+"</td><td>"+record.tip+"</td><td>"+record.name+"</td><td>"+record.account+"</td><td>"+record.sign+"</td><td>"+record.balance+"</td><td>金额:<input type='text' cannext class='money' id='chargeAmount"+record.username+"'/>&nbsp;元<br />回单:<input type='text' cannext id='huidan"+record.username+"' /><br />备注:<input type='text' cannext id='memo"+record.username+"' /><br />专户:<select id='qudao"+record.username+"'><option value='1'>招商银行</option></select>&nbsp;<button type='1' huidan='huidan"+record.username+"' memo='memo"+record.username+"' amount='chargeAmount"+record.username+"' target='"+record.username+"' class='ui-state-default mybutton charge' style='cursor:pointer'>投资人充值</button></td></tr>").appendTo($("#chager_list"));
						}else{
							$("<tr><td>"+record.username+"</td><td>"+record.tip+"</td><td>"+record.name+"</td><td>"+record.account+"</td><td>"+record.sign+"</td><td>"+record.balance+"</td><td>金额:<input type='text' cannext class='money' id='chargeAmount"+record.username+"'/>&nbsp;元<br />回单:<input type='text' cannext id='huidan"+record.username+"' /><br />备注:<input type='text' cannext id='memo"+record.username+"' /><br />专户:<select id='qudao"+record.username+"'><option value='1'>招商银行</option></select><br />类型:<select id='type"+record.username+"'><option value='21'>还款充值</option><option value='22'>履约充值</option></select>&nbsp;<button type='type"+record.username+"' huidan='huidan"+record.username+"' memo='memo"+record.username+"' amount='chargeAmount"+record.username+"' target='"+record.username+"' class='ui-state-default mybutton charge' style='cursor:pointer'>融资方充值</button></td></tr>").appendTo($("#chager_list"));
						}
					});
					$(".money:first").focus();
					init();
