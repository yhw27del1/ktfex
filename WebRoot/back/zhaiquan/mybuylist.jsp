...
function oper(id,title){  
        $.dialog({
			...
			max:  false,
			width: '400px',
		    content: '您要把此条受让记录'+title+'吗?',
		    ok: function(){ 
		            $("#form1").attr("action","/back/zhaiquan/buyingAction!upState");
                    $("#buyId").val(id); 
                    $("#form1").submit(); 
		        return false;
		    },
...
<form action="/back/zhaiquan/buyingAction!mybuylist"  id="form1"> 
					...
					<tr>
						...
						<td>
							<c:if test="${entry.state=='0'}">
								<span style="color: #4169E1;">受让中</span>
							</c:if>
						...
						<td>
							 <c:if test="${entry.state=='1'}">
								<a href="/back/zhaiquan/contractAction!view?id=${entry.contract.id}" target="_blank">查看</a>
							</c:if> 
						</td>
						  <td>
							<c:if test="${entry.state=='0'}">
