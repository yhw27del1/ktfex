...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div id="myToolBar" id="myToolBar" class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			<a href="/sheets/finance!financeList?startDate=<fmt:formatDate value='${startDate}' pattern="yyyy-MM-dd"/>&endDate=<fmt:formatDate value='${endDate}' pattern="yyyy-MM-dd"/>&selectby=${selectby }&financingcode=${financingcode }"><<返回</a>
		</div>
		...
		<table style="margin-top:40px; " class="ui-widget ui-widget-content">
			...
			<tr><td>还款方式</td><td>${result.businessType.returnPattern }</td><td>借款期限</td><td>
					<c:if test="${(result.interestDay)!= 0}">按日计息(${result.interestDay}天)</c:if>
			    <c:if test="${(result.interestDay)== 0}">${result.businessType.term}个月 </c:if>  
		...
		<table style="margin-top:40px;" class="ui-widget ui-widget-content">
			...
			<tbody class="table_solid">
				<c:forEach items="${logs}" var="log" varStatus="sta">
					<tr>
						<td><fmt:formatDate value="${log.time}" pattern="yyyy-MM-dd HH:mm:00"/></td>
						<td>${log.realname}</td>
						<td>${log.operate }</td>
					</tr>
				</c:forEach>  		 
