		...
		function doexport(){
			$.dialog({title:'提示信息',lock:true,min:false,max:false,width:'400px',content:"你确定要导出excel吗？",
				ok:function(){
					...
					var r3 = $("#rate3").val();
					var cl = $("#channel").val();
					window.location.href = "/back/accrual/accrualAction!calc_ex?time="+new Date().getTime()+"&main.memo="+encodeURI(m)+"&main.main_start="+s+"&main.main_end="+e+"&main.main_rate=0"+"&main.interval="+i+"&userType="+ut+"&signType="+st+"&main.sub_start_1="+s1+"&main.sub_end_1="+e1+"&main.sub_rate_1="+r1+"&main.sub_start_2="+s2+"&main.sub_end_2="+e2+"&main.sub_rate_2="+r2+"&main.sub_start_3="+s3+"&main.sub_end_3="+e3+"&main.sub_rate_3="+r3+"&channel="+cl;
				},
				cancelVal:'关闭',
				cancel:true
			});
	...
	<body>
		...
		<div style="margin: 50px auto; font-weight: bold;">
			...
			<div>
				...
				<input id="start3" name="vo2.start3" value="<fmt:formatDate value='${main.sub_start_3}' pattern='yyyy-MM-dd'/>" style="display:none;" />
				<input id="end3" name="vo2.end3" value="<fmt:formatDate value='${main.sub_end_3}' pattern='yyyy-MM-dd'/>" style="display:none;" />
				<input id="rate3" name="vo2.rate3" value="${main.sub_rate_3}" style="display:none;" />
			</div>
			<div>
				...
				<span class="space"></span>
				<span class="title">子期间3：</span><span class="value"><fmt:formatDate value="${main.sub_start_3}" pattern="yyyy-MM-dd"/>至<fmt:formatDate value="${main.sub_end_3}" pattern="yyyy-MM-dd"/></span>
			</div>
			<table style="font-size: 13px; font-weight: bold">
				<thead>
					<tr>
						...
						<th colspan="3" style="text-align:center;">子期间3</th>
						<th rowspan="3">
							积数
						</th>
						<th rowspan="3">
				...
				<tbody>
					<tr>
						...
						<td>
							${dt.sum3}
							<!-- <input name="vo2.sum3" value="${dt.sum3}" style="display:none;" /> -->
						</td>
						<td>
							${dt.r3}%
						</td>
						<td>
							${dt.lx3}
							<!-- <input name="vo2.lx3" username="${dt.username}" class="lxlx" size="10" value="${dt.lx3}" /> -->
						</td>
						<!-- 子期间3 -->
